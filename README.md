...
- **五个内置基本动作**(均在 Unitree A2 上实仿调定): 四节拍行走(0.47 m/s)、对角小跑
  (1.33 m/s)、原地跳跃(起跳 2.96 m/s)、前扑跳跃(起跳 vz 2.03 m/s, 腾空 0.378 s,
  前进 0.267 m)、30 cm 跌落缓冲(冲击峰值 11.9 倍体重, 四足同时触地)
...
**改控制器** (以一次已回退的前扑改造为例, 距离 0.267 -> 0.708 m 但**未采用**):

> 这段是方法论记录, 不是当前实现。改造后的版本已按"过度对称失去参考价值"回退,
> 理由见文末"回退记录"。下面三条机理与改法本身仍然成立, 供后续改控制器时参考。
...
- **反向下沉 (countermovement)** 能加长腿部做功行程 —— 执行器扭矩受限时 (小腿全程
  180 N·m 饱和), 行程越长总冲量越大;
- **空中姿态必须单独控制**: 起跳带的角速度会积分到 20 度以上。滚转用四腿髋外展同向
  摆动做角动量交换; 俯仰同理但**只能给后腿**——前腿足端必须留在腿根正下方才能拿到最大
  *垂直*伸出量 (腿长上限 0.52 m, 足端前移 0.28 m 后垂直分量只剩 0.45 m), 把它摆到前面
  去"控制俯仰"会直接牺牲前腿先着地的能力。
- **一步一验**: 空中控制的符号常常与直觉相反 (本案例里滚转用正号、俯仰用负号才对,
  两者相反), 每次只改一处、用落地时刻/落地俯仰这类可观测量判断, 别一次调多个增益。

### 回退记录: 前扑的"过度对称"

一次把 `leap` 从"对角步态助跑 + 深蹲"改成"bound 助跑 + 跑动中弹射"的改造曾被否决并回退:

| 指标 | v2 原版 (采用) | bound 改造版 (回退) |
| --- | --- | --- |
| 腾空前进距离 | 0.267 m | 0.708 m (2.65x) |
| 左右冲量差 | 对角成对发力, 不近似相等 | **0.39%** |
| 前腿冲量 FL/FR 差 | — | **0.57%** |
| 落地 | 前腿先着地 | 前腿先着地 (+24 ms) |
| 航向漂移 | — | -0.13 度 |

**否决理由**: 0.39% 的左右对称是"理想模型 + 确定性控制器"的产物 —— 模型左右腿几何与
质量完全一致、无装配公差、无传感器噪声、无伺服差异, 真实机器狗不会这么齐。**过度对称的
结果反而失去参考价值**: 它不再能反映真实平台的受力分布。原版对角步态下"一前一后成对
发力"的不对称, 才是时钟步态的真实现象。

**教训**: 做载荷分析时, "把不对称优化掉"不一定是改进 —— 先问这个不对称是**缺陷**
(如控制发散造成的侧翻) 还是**真实特征** (如步态相位决定的对角发力)。前者该修,
后者该留。
...
4. **`leap` 的左右不对称是固有特征**: v2 用对角步态助跑 (FL 与 RR 同相), 起跳与落地
   本就是对角成对发力, 四足受力不近似相等 —— 这是时钟步态的真实现象, 不要当成 bug。
   曾改成 bound 助跑得到 0.39% 左右对称、0.708 m 距离, 但已按"过度对称失去参考价值"回退
   (见文末"回退记录")
