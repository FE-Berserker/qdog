...
| leap | 起跳 vx 1.32 / vz 2.51, 腾空 0.448 s, **前进 0.604 m**, 冲击 7.8×体重 | v3 控制器重写 (原 v2 为 0.267 m); 落地带约 16° 滚转 |
...
3. **包络主导工况是冲击类**: 驱动/弯矩/剪力/轴向四项最大值全部来自 `leap` / `fall` / `drop`,
   ...
   结构件应按 >100% 额定设计
4. **`leap` 落地不对称**: 腾空残留约 16° 滚转 (髋外展角动量控制已把 26° 压到 16°),
   右腿先着地、左腿晚约 0.5 s, 航向漂移 −14.9°
